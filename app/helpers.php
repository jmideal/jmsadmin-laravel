...
use App\Models\System\DeptModel;
...
if (!function_exists('resolveRequest')) {
    function resolveRequest()
    {
        ...
        if (empty($routeAction)) {
            return ["module" => '', "controller" => '', "action" => '', "permission" => ''];
        }
        $routeActionArr = explode('@', $routeAction);
        if (count($routeActionArr) != 2) {
            return ["module" => '', "controller" => '', "action" => '', "permission" => ''];
        }
        $controllerFullClass = $routeActionArr[0];
        $action = $routeActionArr[1];

        $controllerClass = str_replace("App\\Http\\Controllers\\", "", $controllerFullClass);
        $controllerClassArr = explode('\\', $controllerClass);

        $controllersName = getControllerBaseName($controllerClassArr[count($controllerClassArr) - 1]);
        unset($controllerClassArr[count($controllerClassArr) - 1]);
        if (!empty($controllerClassArr)) {
            $controllerClassArr = array_map('lcfirst', $controllerClassArr);
            $controllerClassArr = implode(':', $controllerClassArr);
            $permission = $controllerClassArr . ":" . $controllersName . ":" . $action;
        } else {
            $permission = $controllersName . ":" . $action;
        }
        ...
        return compact("module", "controller", "action", "permission");
    }
}
function getAnnotationPermission(ReflectionClass $reflection = null)
{
    $request = Request();
    $usePermission = [];
    if (empty($reflection) && !empty($request->route()->getController())) {
    ...
    if (!empty($reflection)) {
        ...
        $reflectionMethod = $reflection->getMethod($request->route()->getActionMethod());
...
function getAnnotationLogInfo(ReflectionClass $reflection = null)
{
    ...
    if (!empty($reflection)) {
        ...
        $logInfo['action'] = $request->route()->getActionMethod();
...
if (!function_exists('basicResponse')) {
     ...
     * @return Response
     */
    function basicResponse(string $body = '', int $status = 200, array $headers = []): Response
    {

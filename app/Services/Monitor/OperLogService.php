...
use App\Models\Monitor\OperLogModel;
...
class OperLogService extends BasicService
{
    ...
    public function logInsert($status, $message, $result = '')
    {
        ...
        if (empty($request->route()) || empty($request->route()->getController()) || empty($request->route()->getActionMethod())) {
            return true;
        }
        $reflection = new ReflectionClass($request->route()->getController());
        if (getControllerBaseName($reflection->getShortName()) == 'auth' && in_array($request->route()->getActionMethod(), ['login', 'logout'])) {
            return true;
        }

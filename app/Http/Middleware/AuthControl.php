...
class AuthControl
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        //是否需要判断演示模式
        if (getControllerBaseName($reflection->getShortName()) != 'auth' && !in_array($request->route()->getActionMethod(), $noNeedLogin)) {
        ...
        // 访问的方法需要鉴权
        if (!in_array('*', $noNeedAuth) && !in_array($request->route()->getActionMethod(), $noNeedAuth)) {
            //用户未登录
            if (empty($user)) {
